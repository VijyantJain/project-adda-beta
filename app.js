...
function renderShortResult(d,j){clearInterval(pollTimer);const entries=j.result?.entries||[];app.innerHTML=shell(`${top(crew.name,'crew')}<div class="row between"><span class="chip">${labelType(d.type)}</span>${creatorTools(d)}</div><div class="sp12"></div><div class="questionHero"><span>💬 LIVE ANSWERS</span><h1>${esc(d.question)}</h1><p>${entries.length} repl${entries.length===1?'y':'ies'} so far</p></div><div class="sp12"></div><div class="answerStack">${entries.map(r=>`<div class="answerCard"><b>${esc(r.nickname)}</b><p>${esc(r.answer)}</p></div>`).join('')}</div><div class="sp12"></div><button class="btn primary" onclick="window._shareDrop('${d.id}')">Share with friends</button>`);pollTimer=setInterval(()=>{if(screen==='drop')renderDrop()},3000)}
