...
async function renderCrewSettings(){
  ...
  app.innerHTML=shell(`${top('Crew settings','crew')}<div class="card"><div class="row between"><div><div class="tiny">CREW</div><h2>${esc(crew.name)}</h2></div><span class="chip">${admin?'Admin':'Mate'}</span></div>${admin?`<div class="sp12"></div><div class="field"><label>Rename Crew</label><div class="row"><input id="renameCrew" value="${esc(crew.name)}"><button class="miniPrimary" onclick="window._renameCrew()">Save</button></div></div>`:''}</div><div class="sp12"></div><div class="card"><div class="row between"><h3>Crew mates</h3><span class="chip">${matesLabel()}</span></div><div class="sp12"></div>${members.map(m=>`<div class="settingsRow memberRow"><span class="memberName">${avatarHtml(m.nickname)}<span>${esc(m.nickname)}${m.id===crew.createdBy?' · Admin':''}</span></span>${admin&&m.id!==pid?`<button class="dangerMini" onclick="window._removeMember('${m.id}')">Remove</button>`:''}</div>`).join('')}</div><div class="sp12"></div><button class="btn ghost" onclick="window._shareCrew()">Share Crew invite</button><div class="sp12"></div>${admin?`<button class="btn ghost" onclick="window._go('crewInsights')">Field-test insights</button><div class="sp12"></div><button class="btn dangerBtn" onclick="window._deleteCrew()">Delete Crew</button>`:`<buttton class="btn dangerBtn" onclick="window._leaveCrew()">Leave Crew</button>`}`)
}
...
function renderJoin(){app.innerHTML=shell(`${top(crew?.name||'Join Crew')}<div class="hero"><span class="chip darkchip">INVITE ONLY</span><div class="sp18"></div><h1>${esc(crew.name)}</h1><p style="color:#cbc5d2;margin-top:8px">${matesLabel()} are here.</p></div><div class="sp18"></div><div class="card"><div class="field"><label>What should your Crew call you?</label><input id="joinName" maxlength="24" value="${esc(meName)}" placeholder="Your name"></div><div class="sp18"></div><button class="btn primary" onclick="window._joinCrew()" >Join the Crew</button><p class="sub center" style="margin-top:9px">Become a mate and jump in.</p></div>`,false)}
...
function renderCreate(){
  ...
  app.innerHTML=shell(`${top('Create a Drop','crew')}<div class="createHero"><div><h1>What do you want from the Crew?</h1></div><span class="sparkle">✦</span></div><div class="sp12"></div><div class="formats">${cards.map(x=>`<button data-type="${x[0]}" class="format ${type===x[0]?'on':''} ${x[0]==='likely'&&members.length<2?'locked':''}" onclick="window._type('${x[0]}')"><span class="fic">${x[1]}</span><span class="ftxt"><b>${x[2]}</b><small>${x[3]}</small></span></button>`).join('')}</div>${members.length<2?'<p class="sub" style="margin-top:7px">Most Likely unlocks after one friend joins.</p>':''}<div class="sp12"></div><div class="card createCard"><div class="field"><div class="row between"><label>${questionLabel(type)}</label><button type="button" class="surpriseBtn" onclick="window._surprise('${type}')">🎲 Surprise me</button></div><textarea id="q" maxlength="120" placeholder="${placeholder(type)}">${esc(selected?.question||'')}</textarea></div>${extraFields(type)}${settings}<div class="sp18"></div><button class="btn primary" onclick="window._publish()">Create & share</button></div>`);
}
...
window._type=t=>{if(t==='likely'&&members.length<2)return toast('Most Likely unlocks after one friend joins');const prev=selected?.type||(members.length<2?'short':'likely');if(prev!==t)resetMedia();selected={type:t,question:$('#q')?.value||''};renderCreate()};
