...
function renderCreate(){
  ...
  const settings=type==='short'?'':`<div class="sp12"></div><div class="field"><label>Reveal after…</label><div class="revealChoices"><button class="${window._thresholdCount===2?'on':''}" onclick="window._setCount(2)">2 answers</button><button class="${!window._thresholdCount||window._thresholdCount===3?'on':''}" onclick="window._setCount(3)">3 answers</button><button class="${window._thresholdCount===5?'on':''}" onclick="window._setCount(5)">5 answers</button><button class="${window._thresholdMode==='everyone'?'on':''}" onclick="window._setEveryone()">Everyone here now</button></div><p class="sub" style="margin-top:6px">“Everyone here now” means members already in the Crew when you create the Drop. New joiners won’t block the reveal.</p><button class="moreSettingsBtn" onclick="window._toggleDropSettings()">⚙ More settings</button><div id="dropSettings" class="dropSettings ${window._showDropSettings?'open':''}"><label class="toggleRow"><span><b>Show who picked what</b><small>Names become visible after the reveal.</small></span><input id="showNames" type="checkbox" ${window._showNames?'checked':''} onchange="window._showNames=this.checked"></label><label class="toggleRow"><span><b>Let people change answers</b><small>Until the reveal unlocks.</small></span><input id="allowChange" type="checkbox" ${window._allowChange===false?'':'checked'} onchange="window._allowChange=this.checked"></label></div></div>`;
...
window._setCount=n=>{window._thresholdMode='count';window._thresholdCount=n;renderCreate()};
window._setEveryone=()=>{window._thresholdMode='everyone';window._thresholdCount=null;renderCreate()};
...
window._publish=async()=>{
  ...
  const body={crewId,participantId:pid,type,question,thresholdMode:window._thresholdMode||'count',thresholdCount:window._thresholdCount||3,showNames:window._showNames===true,allowChange:window._allowChange!==false};
  if(type==='either'||type==='predict'){body.optionA=$('#a')?.value;body.optionB=$('#b')?.value}
  if(type==='vote'){body.options=[$('#o1')?.value,$('#o2')?.value,$('#o3')?.value]}
  try{const j=await api('createDrop',{method:'POST',body});track('drop_created',{type});dropId=j.drop.id;history.replaceState({},'',`/?crew=${crewId}&drop=${dropId}`);screen='drop';selected=null;window._thresholdMode='count';window._thresholdCount=3;window._showNames=false;window._allowChange=true;window._showDropSettings=false;render();setTimeout(()=>window._shareDrop(dropId),500)}catch(e){toast(e.message)}
};
...
window._submitAnswer=async()=>{const answer=activeDropType==='short'?$('#shortAnswer')?.value.trim():selected;if(!answer)return toast('Add your answer');try{await api('answerDrop',{method:'POST',body:{crewId,dropId,participantId:pid,answer}});track('response_submitted');selected=null;renderDrop()}catch(e){toast(e.message)}};
...
function renderShortResult(d,j){clearInterval(pollTimer);const entries=j.result?.entries||[];app.innerHTML=shell(`${top(crew.name,'crew')}<div class="row between"><span class="chip">${labelType(d.type)}</span>${creatorTools(d)}</div><div class="sp12"></div><h1>${esc(d.question)}</h1><p class="sub" style="margin-top:7px">Answers appear live.</p><div class="sp12"></div><div class="answerStack">${entries.map(r=>`<div class="answerCard"><b>${esc(r.nickname)}</b><p>${esc(r.answer)}</p></div>`).join('')}</div><div class="sp12"></div><button class="btn ghost" onclick="window._shareDrop('${d.id}')">Share question</button>`);pollTimer=setInterval(()=>{if(screen==='drop')renderDrop()},3000)}
function renderResult(d,j){const byId=Object.fromEntries(d.options.map(o=>[o.id,o.label]));const first=j.result?.ranked?.[0];const pct=first?Math.round(first.count/j.result.total*100):0;app.innerHTML=shell(`${topbarResult()}<div class="row between"><span class="chip">${labelType(d.type)}</span>${creatorTools(d)}</div><div class="sp12"></div><div class="hero center"><div style="font-size:44px">✨</div><div class="sp12"></div><div class="resultNum">${pct}%</div><h1>${esc(byId[first?.answer]||'Result')}</h1><p style="color:#cbc5d2;margin-top:8px">${esc(d.question)}</p></div><div class="sp18"></div>${j.result.ranked.map(r=>`<div class="card" style="margin-bottom:8px"><div class="row between"><b>${esc(byId[r.answer]||r.answer)}</b><span>${r.count}</span></div></div>`).join('')}<div class="sp12"></div><button class="btn primary" onclick="window._go('create')">Make the next Drop</button><div class="sp12"></div><button class="btn ghost" onclick="window._shareDrop('${d.id}')">Share Drop</button>`)}
