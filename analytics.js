...
function filteredVisitors(){
  ...
  return (D.visitors||[]).filter(v=>[v.name,v.participantId,v.ip,v.city,v.region,v.country,v.browser,v.os,v.deviceType,v.source,v.language,v.timezone,v.trafficClass,v.entryType].join(' ').toLowerCase().includes(q))
}
...
function visitorTableInner(rows){
  return `<div class="tableWrap"><table class="visitorTable"><thead><tr><th>Visitor</th><th>Location / IP</th><th>Device</th><th>Sessions</th><th>Answers</th><th>Source</th><th>Traffic signal</th><th>Last seen</th></tr></thead><tbody>${rows.map(v=>`<tr onclick="showVisitor('${esc(v.participantId)}')"><td><b>${esc(v.name||'Unnamed')}</b><small>${esc(v.participantId)}</small></td><td><b>${esc([v.city,v.region,v.country].filter(Boolean).join(', ')||'Unknown')}</b><small>${esc(v.ip||'No IP')}</small></td><td><b>${esc(v.deviceType||'Unknown')} · ${esc(v.browser||'')}</b><small>${esc(v.os||'')} ${v.platform?`· ${esc(v.platform)}`:''}</small></td><td>${n(v.sessions)}<small>${n(v.visits)} loads</small></td><td>${n(v.answerCount)}<small>${n(v.dropCreates)} created</small></td><td>${esc(v.source||'Direct / unknown')}</td><td><b>${esc(v.trafficClass||'Not assessed')}</b><small>${esc(v.entryType||'')}</small></td><td>${ago(v.lastSeen)}<small>${fmt(v.lastSeen)}</small></td></tr>`).join('')}</tbody></table></div>`
}
...
window.showVisitor=id=>{
  ...
  ${detail('Network',v.connection?.effectiveType?`${v.connection.effectiveType} · ${v.connection.downlink||0} Mbps · ${v.connection.rtt||0} ms`:'—')}${detail('Device memory',v.deviceMemory?`${v.deviceMemory} GB`:'—')}${detail('CPU threads',v.hardwareConcurrency||'—')}${detail('Source',v.source||'Direct / unknown')}${detail('Traffic classification',v.trafficClass||'Not assessed')}${detail('Entry type',v.entryType||'Unknown')}${detail('Engagement',v.engagementClass||'Unknown')}${detail('Days observed',n(v.daysObserved||0))}
