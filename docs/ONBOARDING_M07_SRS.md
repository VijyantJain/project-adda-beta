...
## v0.7.1 field-test correction (2026-09-23)
Direct Tenacious ≠ app conclusion or forced invitation. Primary action: Explore my Aura, showing earned real Starter progress. Optional separate Create Crew link remains; Home/Profile/Aura operate with no Crew. Invited cohort keeps original Crew/Drop entry. Guided first Crew and five-tab walkthrough cannot skip or exit by incidental overlay/background taps: only coach-card buttons respond, with an explicit real answer phase allowing options, input and Submit. Short simple copy; no compulsory photo/gender/bio or made-up verification. Profile wizard modal owns its fields: photo OR emoji/avatar OR default, optional bio, optional unreserved handle draft, display name, Save; browser-local only. Refresh/resume and a clear failure-retry path must be verified on actual devices. Per-user Crew list badges read existing myResponse and prioritize unanswered. Aura is DISPLAY name; preserve getVibe, /?profile, analytics IDs and all stored scores. One-time profile-avatar points mutually exclusive with photo points. Add tenacious-solo cohort tracking. Future Daily Aura Deck separate v0.8 acceptance, not implied live.

### Guided tour edge-case requirements
Explicitly permit private interest-pack selection, optional gender selector, Surprise Me and private answer buttons while denying underlying Crew navigation. If fewer than two unsubmitted REAL Drops remain (especially replay on a fully answered Crew), continue to tab coach marks and record shortage rather than fabricating answers, resetting results, or creating an unavoidable loop. Crew card must show both per-user completion and response/reveal progress.
...
Aura copy acceptance also covers backend-supplied Starter facts, the earned milestone formerly displayed Vibe Builder, and the level formerly displayed Vibe Magnet. No changes to stored answer IDs, milestone thresholds, score amounts or Blob keys.
