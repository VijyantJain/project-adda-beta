...
B. First-time via CREW link with EXISTING Drops:
  ...
  4. REQUIRED: after two genuine Crew answers, play all ten Starter Drops → Tenacious → complete Aura/Home/Crew/Create informational steps and step-by-step Profile guided setup; keep the original invited crewId and actual two Crew answer records intact.
  5. Show “Now create your first Drop in [exact crew.name]”. Automatically open the + Create Drop screen INSIDE THAT SAME CREW, run the complete first-custom-Drop guide (all six formats, eligibility, fields/settings, publish), and persist the resulting real Drop in the original Crew.
  6. Offer a native/fallback share of the **new Drop's exact deep link**, acknowledge cancellation honestly, finish the guided mission, and resume normal Crew and Adda use. No phantom Crew creation, no auto-seeded pack for a joined Crew, no duplicate authored Drop on timeout/refresh.
...
8. Crew invite new with 2+ available: exact Crew/member guide → two real Drop answers → required Starter 10/Tenacious/Aura/Profile → original Crew → required first-custom-Drop guided creation through + → actual published Drop in original Crew → exact Drop share offer → normal use. No phantom Crew or double answer/publish.
