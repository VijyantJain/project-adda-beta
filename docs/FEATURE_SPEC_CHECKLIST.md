...
RESOLVED (D038): new Crew-link entrant completes two actual Crew Drop answers, then mandatory Starter 10/Tenacious/Aura/Profile, then exact original Crew, required first-custom-Drop guided + tour and real publish in original Crew, share offer, then normal use. v0.7.2 unchanged until implemented. New Drop-link entrant explicitly answers invited Drop before Starter as founder requested.
