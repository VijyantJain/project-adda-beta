...
## v0.6.2 correction — ten-card completion is compulsory for both first-time cohorts
For *both* brand-new direct visitors and brand-new invitees, initial welcome is visually identical "PLAY FIRST · CREW LATER / 5 quick ones. Find your Vibe". Invitation is recorded provisionally in background but does NOT automatically trigger the first question or bypass welcome. Five-card milestone offers **Play my next Five** only. After ten, Tenacious shows earned 180 Starter Vibe/6 badges and cohort-specific primary action: direct visitor → Create Crew; invited visitor → Enter linked Crew/Drop after name. Only then first-Crew opt-skippable interactive tutorial explains Crew, Drop, answer and guides 1–2 real Drop submissions. A returning named member retains direct link routing. Every browser is not necessarily a unique lifetime user; verified identity is M1.

## v0.7.2 content version v3 and realism upgrade
NEW Starter records use deckVersion=v3, legacy v1 and v2 remain unchanged. Fixed IDs s1–s10 and consistent s6 interest selection; new situational copy focuses on late friends, 9-minute voice notes, low-battery cab panic, chai-only bill splits, group-chat promises, seven explicit interests, plans and real friend superpowers. s7/s8 select one of seven × two revised interest prompts. Twenty-four bundled photographic local WebP scenes now replace repeated silhouette graphics; two additional private-guide cards also use appropriate interest images. On a failed photo load, existing locally bundled SVG remains a usable visual fallback. Version compatibility: old v1/v2 answer option strings and rewards unchanged; v3 new cohort answers count 10×10 +30 +50=180. Refresh during bonus six through nine resumes the current question rather than sending user back to First Five. Tenacious direct moves to scored Aura+guided tour; invited to original shared Crew/Drop. D-020/D-021 high-resolution masters, source license and release clearance remain IN_PROGRESS.
